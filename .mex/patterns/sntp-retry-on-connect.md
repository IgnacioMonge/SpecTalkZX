...
- `sntp_process_response` validates each parsed hour/minute/second immediately before storing it. Do not reintroduce the old final reload-and-range-check block; it costs resident bytes and can leave more partially-invalid time state before rejecting a bad response.
