...
- After subtracting `_ring_buffer` from an overlay entry pointer, reject `HL >= 2048` with `ld a,h / cp 8` rather than another 16-bit subtract. The offset is already isolated, so the 2K boundary is exactly `H >= 8`.
