...
- If a received byte must remain in `A` while clearing a memory latch, prefer `ld hl,latch / ld (hl),0` over shuttling through `E`, provided `HL` is not part of the helper's return contract. Do not replace it with `xor a / ld (latch),a` in `uartRead`: that clobbers the returned byte in `A`.
