...
- If a MODE query uses numeric replies like `324`, surface a visible reply through `notify()`; updating only local status-bar state makes the feature feel broken even when the wire protocol is correct, while raw main-area output bypasses `!notif`.
