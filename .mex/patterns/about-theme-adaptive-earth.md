...
- The current bitmap set is copied from the compatible `GloboTerraqueoZX\build` output. Do not substitute the older `GloboTerraqueoZX\SpectalkZX` export without changing frame count/compact size. The tiny lower-right edge sparkle mask is still kept as a defensive hardware cleanup until a full theme pass proves it is obsolete.
