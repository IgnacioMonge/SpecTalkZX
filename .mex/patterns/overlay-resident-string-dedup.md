...
## Rejected
- `overlay/spectalk_ovl2.c` must not reuse `SB_ON`, `SB_OFF`, `SB_NOTSET`, or `SB_SMART` for config values because it renders them with `print_str64()`. HW showed missing characters in `(not set)` and `smart`.
