...
- If the overlay command needs the command argument, copy it into `overlay_slot` before `overlay_exec()`. Do not use `search_pattern` for transient command args, because it also carries saved-session/autojoin state across disconnected `!save` paths.
...
- `overlay/spectalk_ovl3.c`
- `overlay/overlay_entry.asm`
- `overlay/overlay_entry3.asm`
