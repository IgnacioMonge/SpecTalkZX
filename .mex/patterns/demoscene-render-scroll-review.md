...
- The accepted `_print_line64_fast()` fast path detects pairs that normalize to blank (space, NUL padding, control byte, or high byte), skips both `unpack_glyph` calls and the left-buffer `LDIR`, writes all 8 scanlines as zero, preserves NUL pointer semantics, and leaves attribute fill unchanged. Current cost: `+51B` after shrink created enough slack.
...
- The accepted scroll micro-optimization specializes final chat-row cleanup for row 19 using fixed bitmap/attribute addresses (`0x5060` and `0x5A60`) instead of calling `cli_internal()`. Current cost: `+28B`; row 20 notification/status/input rows remain outside the touched area.
