...
- This paid off in `asm/spectalk_asm/80_ui_runtime.asm`: `cfg_kv()` shrank by reusing `_cfg_put()` for both the `key` and string `val` copies, and `sys_puts_print()` shrank by calling `_sys_puts()` directly instead of restating `set_attr_sys()+main_puts()`.
