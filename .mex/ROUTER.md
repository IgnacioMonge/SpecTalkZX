...
## Project State
- Date: 2026-04-22
- Recent task: completed `/mode` help integration. The command already worked, but its help entry lived only in the DAT payload; help text now comes from tracked `src/SPECTALK_HELP.txt`, and `/mode` is listed there.
...
- Current verified TAP from `make`: `36025B` on 2026-04-22 (`+164B` vs the previous 35,861B build, `-47B` vs `v1.3.7`).
- Current overlay sizes from verified `make`: `SPCTLK1.OVL` 1385B, `SPCTLK2.OVL` 1968B, `SPCTLK3.OVL` 1591B, `SPCTLK4.OVL` 1796B.
- Audit highlight: the long-`ABOUT` disconnect hole remains fixed, and `/mode` now has a usable minimum UX for channel queries without introducing a full parser.
