...
# Source files (Single Compilation Unit)
C_SOURCES = src/main_build.c
# Individual C files (for dependency tracking)
C_MODULES = src/spectalk.c src/irc_handlers.c src/user_cmds.c
...
SpecTalkZX.tap: $(C_SOURCES) $(C_MODULES) $(ASM_COMMON) asm/divmmc_uart.asm $(HEADERS)
...
SpecTalkZX_AY.tap: $(C_SOURCES) $(C_MODULES) $(ASM_COMMON) asm/ay_uart.asm $(HEADERS)
	$(ZCC) $(PLATFORM) $(CFLAGS_COMMON) $(PRAGMAS) $(LDFLAGS) \
		$(C_SOURCES) asm/ay_uart.asm $(ASM_COMMON) \
