...
Writes: overlay/whatsnew_data.h and build/whatsnew_logo.bin

The packed bitmap lives in SPECTALK.DAT (same pattern as ABOUT Earth).
whatsnew_data.h keeps version, dimensions, and changelog strings only.
...
def convert_logo(png_path, max_w=96, max_h=88, threshold=160):
    """Convert PNG to monochrome ZX bitmap. Returns (bytes, width_bytes, height)."""
    try:
        from PIL import Image

        img = Image.open(png_path).convert("L")
        # Never upscale a release asset: a 64px bust must not be stretched to 96px.
        ratio = min(1.0, min(max_w / img.width, max_h / img.height))
        ...
        ratio = min(1.0, min(max_w / width, max_h / height))
...
def main():
    ...
    # Packed bitmap is streamed from SPECTALK.DAT; keep it out of SPCTLK3.
    bin_path = Path("build/whatsnew_logo.bin")
    bin_path.parent.mkdir(exist_ok=True)
    bin_path.write_bytes(logo_packed)

    # Generate C header (metadata + changelog only)
    lines = [
    ...
    print(
        ...
        f"({len(logo_data)}B raw, {len(logo_packed)}B packed -> {bin_path}), "
